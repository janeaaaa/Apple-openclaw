...
print('[INFO] Creating new cover with full title...')
...
# 绘制主标题 - 更大更完整
draw.text((50, 100), 'DeepSeek', fill=(255, 255, 255), font=font_large)
draw.text((50, 180), 'DeepSeek', fill=(180, 180, 180), font=font_small)
draw.text((50, 250), 'DeepSeek', fill=(140, 140, 160), font=font_small)
draw.text((50, 330), 'DeepSeek', fill=(100, 100, 120), font=font_small)
...
with urllib.request.urlopen(req, timeout=30) as resp:
    ...
    if 'url' in r:
        ...
        with urllib.request.urlopen(req2, timeout=30) as resp2:
            ...
            # 创建DeepSeek草稿
            print('[INFO] Recreating DeepSeek draft...')
            
            title = '国产AI突然炸锅！DeepSeek凭啥让硅谷慌了？'
            digest = '一个名不见经传的公司，怎么突然成了全网焦点？'
...
<p style="font-size:16px;color:#333333;margin-bottom:10px;"><strong>机会一：AI从业者更值钱了</strong></p>
...
<p style="font-size:14px;color:#999999;text-align:center;">如果觉得有用，点个「在看」，让更多人看到中国AI的崛起。</p>'''
            ...
            with urllib.request.urlopen(req3, timeout=30) as resp3:
                ...
                if 'media_id' in r3:
                    print('[OK] SUCCESS!')
                    print('New Draft Media ID:', r3['media_id'])
                else:
                    print('[ERROR]', r3)
